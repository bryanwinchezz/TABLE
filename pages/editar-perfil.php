...
// Salvar chave de API via AJAX silencioso (Qualidade Absoluta)
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['salvar_chave_api_ajax'])) {
    header('Content-Type: application/json');
    if (!isset($_SESSION['usuario'])) {
        echo json_encode(['success' => false, 'error' => 'Usuário não autenticado']);
        exit;
    }
    $nova_chave = trim($_POST['api_key_gemini'] ?? '');
    try {
        $conn = Database::getConexao();
        
        // Garante que a coluna existe caso ela ainda não tenha sido criada
        try {
            $chkGemini = $conn->query("SHOW COLUMNS FROM tb_usuario LIKE 'ds_api_key_gemini'");
            if ($chkGemini->rowCount() === 0) {
                $conn->exec("ALTER TABLE tb_usuario ADD COLUMN ds_api_key_gemini VARCHAR(255) DEFAULT NULL");
            }
        } catch (Exception $e) {}

        $stmt = $conn->prepare("UPDATE tb_usuario SET ds_api_key_gemini = :key, dt_atualizacao = CURRENT_TIMESTAMP WHERE id_usuario = :id");
        $stmt->execute([
            ':key' => !empty($nova_chave) ? $nova_chave : null,
            ':id' => $_SESSION['usuario']['id']
        ]);
        $_SESSION['usuario']['api_key_gemini'] = !empty($nova_chave) ? $nova_chave : null;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['salvar_dados'])) {
    ...
    $nova_chave_api = trim($_POST['api_key_gemini'] ?? '');
        ...
        try {
            $stmt = $conn->prepare("
                ...
                SET nm_usuario = :nome, 
                    ...
                    ds_email = :email,
                    ds_bio = :bio,
                    ds_api_key_gemini = :api_key_gemini,
            ...
            $stmt->execute([
                ...
                ':email' => $novo_email,
                ':bio' => trim($_POST['bio'] ?? ''),
                ':api_key_gemini' => !empty($nova_chave_api) ? $nova_chave_api : null,
                ':id' => $_SESSION['usuario']['id']
            ]);
            ...
            $_SESSION['usuario']['api_key_gemini'] = !empty($nova_chave_api) ? $nova_chave_api : null;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['virar_mestre'])) {
    try {
        $stmtCargo = $conn->prepare("SELECT tp_cargo FROM tb_usuario WHERE id_usuario = :id LIMIT 1");
        $stmtCargo->execute([':id' => $_SESSION['usuario']['id']]);
        $cargoAtual = $stmtCargo->fetchColumn();

        $novoCargo = ($cargoAtual === 'admin') ? 'admin' : 'mestre';

        $stmt = $conn->prepare("UPDATE tb_usuario SET tp_cargo = :cargo, fl_plano_completo = 1, fl_plano_mapas = 1, fl_plano_sistemas = 1 WHERE id_usuario = :id");
        $stmt->execute([':cargo' => $novoCargo, ':id' => $_SESSION['usuario']['id']]);
        $_SESSION['usuario']['cargo'] = $novoCargo;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['desistir_mestre'])) {
    try {
        ...
        $novoCargo = ($cargoAtual === 'admin') ? 'admin' : 'jogador';

        // 2. Mudar o cargo do usuário para o novo cargo e registrar a desistência e zerar os planos
        $stmt = $conn->prepare("UPDATE tb_usuario SET tp_cargo = :cargo, dt_desistencia_mestre = NOW(), fl_plano_mapas = 0, fl_plano_sistemas = 0, fl_plano_completo = 0 WHERE id_usuario = :id");
        $stmt->execute([':cargo' => $novoCargo, ':id' => $_SESSION['usuario']['id']]);
...
try {
    $stmt = $conn->prepare("
        SELECT id_usuario, nm_usuario, ds_email, dt_nascimento, dt_cadastro, tp_cargo, ds_foto, ds_bio, ds_api_key_gemini, dt_desistencia_mestre 
        FROM tb_usuario 
        WHERE id_usuario = :id 
        LIMIT 1
    ");
...
<head>
    ...
    <style>
        /* Animações Premium para os Modais de Perfil */
        ...
        <div class="cartao-edicao">
            ...
            <section>
                ...
                <form class="formulario-edicao" method="POST">
                    ...
                    <div class="campo-edicao" id="secao-gemini-key">
                        <label for="input-api-key-gemini" style="display: flex; justify-content: space-between; align-items: center;">
                            <span>Chave API do Gemini (Inteligência Artificial):</span>
                            <button type="button" class="btn-tutorial-link" onclick="abrirModalTutorial()" style="background: none; border: none; color: #7b4ff7; font-size: 0.8rem; font-weight: 700; cursor: pointer; text-decoration: underline; padding: 0;">Como obter?</button>
                        </label>
                        <div class="input-password-wrapper" style="position: relative;">
                            <input type="password" id="input-api-key-gemini" name="api_key_gemini"
                                value="<?= htmlspecialchars($usuario['ds_api_key_gemini'] ?? '') ?>" 
                                placeholder="Insira sua chave da API Gemini do Google AI Studio..."
                                style="padding-right: 45px;">
                            <button type="button" id="btn-toggle-gemini-key" onclick="toggleGeminiKeyVisibilidade()"
                                style="position: absolute; right: 10px; top: 50%; transform: translateY(-50%); background: none; border: none; color: #888; cursor: pointer; padding: 5px; font-size: 1.1rem; display: flex; align-items: center; justify-content: center;">
                                <i class="fas fa-eye" id="icon-eye-gemini"></i>
                            </button>
                        </div>
                    </div>
            ...
            <section>
                <h3 class="titulo-roxo"><?= (strtolower($usuario['tp_cargo']) === 'mestre' || (strtolower($usuario['tp_cargo']) === 'admin' && $usuario['dt_desistencia_mestre'] === null)) ? 'Mestre' : 'Ser Mestre' ?>
                </h3>
                <p class="texto-descricao">
                    <?= (strtolower($usuario['tp_cargo']) === 'mestre' || (strtolower($usuario['tp_cargo']) === 'admin' && $usuario['dt_desistencia_mestre'] === null))
                        ? 'Você é um Mestre! Possui acesso à criação de campanhas, sistemas e mundos. Se decidir que não quer mais essa responsabilidade, você pode voltar a ser um jogador/administrador comum a qualquer momento.'
                ...
                <div class="area-botoes-centro">
                    <?php if (strtolower($usuario['tp_cargo']) === 'mestre' || (strtolower($usuario['tp_cargo']) === 'admin' && $usuario['dt_desistencia_mestre'] === null)): ?>
...
<script>
    ...
    // Lógica do Cropper para foto de perfil
    ...
    // Lógica para controle da Chave de API do Gemini
    function toggleGeminiKeyVisibilidade() {
        const input = document.getElementById('input-api-key-gemini');
    ...
    function abrirModalTutorial() {
        document.getElementById('modal-tutorial-gemini').style.display = 'flex';
    }

    function fecharModalTutorial() {
        document.getElementById('modal-tutorial-gemini').style.display = 'none';
    }

    // Scroll e foco automático na chave Gemini
    window.addEventListener('load', () => {
        const urlParams = new URLSearchParams(window.location.search);
        if (urlParams.get('foco') === 'gemini-key' || window.location.hash === '#gemini-key') {
            const secao = document.getElementById('secao-gemini-key');
            if (secao) {
                secao.scrollIntoView({ behavior: 'smooth', block: 'center' });
                const inputKey = document.getElementById('input-api-key-gemini');
                if (inputKey) {
                    inputKey.focus();
                    inputKey.style.borderColor = '#7b4ff7';
                    inputKey.style.boxShadow = '0 0 10px rgba(123, 79, 247, 0.3)';
                }
            }
        }
    });

    // Salvar chave Gemini de forma silenciosa e em tempo real
    const inputGeminiKey = document.getElementById('input-api-key-gemini');
    if (inputGeminiKey) {
        inputGeminiKey.addEventListener('change', async function() {
            const val = inputGeminiKey.value.trim();
            const formData = new FormData();
            formData.append('salvar_chave_api_ajax', '1');
            formData.append('api_key_gemini', val);
            
            // Indicação visual de salvamento em andamento
            inputGeminiKey.style.borderColor = '#7b4ff7';
            inputGeminiKey.style.boxShadow = '0 0 10px rgba(123, 79, 247, 0.3)';
            
            try {
                const resp = await fetch('editar-perfil.php', {
                    method: 'POST',
                    body: formData
                });
                ...
                if (data && data.success) {
                    // Feedback de sucesso
                    inputGeminiKey.style.borderColor = '#2ecc71';
                    inputGeminiKey.style.boxShadow = '0 0 10px rgba(46, 204, 113, 0.3)';
                    setTimeout(() => {
                        inputGeminiKey.style.borderColor = '';
                        inputGeminiKey.style.boxShadow = '';
                    }, 1500);
                } else {
                    inputGeminiKey.style.borderColor = '#e74c3c';
                    inputGeminiKey.style.boxShadow = '0 0 10px rgba(231, 76, 60, 0.3)';
                }
            } catch (err) {
                console.error(err);
                inputGeminiKey.style.borderColor = '#e74c3c';
...
<!-- MODAL: TUTORIAL API GEMINI (GOOGLE AI STUDIO) -->
<div id="modal-tutorial-gemini"
    ...
    <div
        style="background:#1e1b26; padding:30px; border-radius:16px; width:90%; max-width:550px; color:#fff; text-align:left; border: 2px solid #7b4ff7; box-shadow: 0 10px 30px rgba(123, 79, 247, 0.3); font-family: 'Montserrat', sans-serif;">
        <h2 style="color:#7b4ff7; display:flex; align-items:center; gap:12px; margin-top:0; font-size:1.6rem; font-weight:800;">
            <i class="fas fa-wand-magic-sparkles"></i> Chave da API do Gemini
        </h2>
        <p style="color:#ccc; font-size:0.9rem; line-height:1.5; margin-bottom:20px;">
            A inteligência artificial no TABLE é alimentada diretamente pela sua própria chave de API. Isso garante que a ferramenta seja <strong>gratuita</strong> e sem limites arbitrários para você!
        </p>
        <div style="background:rgba(255,255,255,0.05); padding:15px; border-radius:10px; margin-bottom:20px;">
            <h4 style="margin:0 0 10px 0; color:#fff; font-size:0.95rem; font-weight:700;">Passo a Passo para Obter sua Chave:</h4>
            <ol style="margin:0; padding-left:20px; color:#ddd; font-size:0.85rem; line-height:1.6;">
                <li style="margin-bottom:8px;">Acesse o site oficial do <a href="https://aistudio.google.com/" target="_blank" style="color:#7b4ff7; font-weight:700; text-decoration:underline;">Google AI Studio</a>.</li>
                <li style="margin-bottom:8px;">Faça login com a sua conta Google (Gmail comum).</li>
                <li style="margin-bottom:8px;">Clique no botão azul destacado <strong>"Get API key"</strong> no menu superior esquerdo.</li>
                <li style="margin-bottom:8px;">Clique em <strong>"Create API key"</strong>, escolha se deseja criar em um projeto existente ou em um novo.</li>
                <li style="margin-bottom:8px;">Copie a chave gerada (ela começa com <code>AIzaSy</code>) e cole-a no campo correspondente nas suas configurações de perfil do TABLE.</li>
            </ol>
        </div>
        <div style="text-align:right;">
            <button onclick="fecharModalTutorial()"
                style="padding:10px 25px; background:#7b4ff7; color:white; border:none; border-radius:8px; cursor:pointer; font-weight:700; transition: transform 0.2s;"
