...
try {
    ...
    try {
        ...
        // Alterar ds_abreviacao de VARCHAR(10) para VARCHAR(50) na tb_atributo
        $pdo->exec("ALTER TABLE tb_atributo MODIFY COLUMN ds_abreviacao VARCHAR(50) DEFAULT NULL");
        
        // Alterar tp_item de ENUM para VARCHAR na tb_item
        $pdo->exec("ALTER TABLE tb_item MODIFY COLUMN tp_item VARCHAR(50) NOT NULL DEFAULT 'outro'");
        
        // Coluna ds_deslocamento na tb_personagem
        $chkDesl = $pdo->query("SHOW COLUMNS FROM tb_personagem LIKE 'ds_deslocamento'");
        if ($chkDesl->rowCount() === 0) {
            $pdo->exec("ALTER TABLE tb_personagem ADD COLUMN ds_deslocamento VARCHAR(50) DEFAULT '9 m / 6 q'");
        }
        
        // Coluna ds_atributo_defesa na tb_personagem para salvar a preferência de atributo
        $chkAttrDef = $pdo->query("SHOW COLUMNS FROM tb_personagem LIKE 'ds_atributo_defesa'");
        if ($chkAttrDef->rowCount() === 0) {
            $pdo->exec("ALTER TABLE tb_personagem ADD COLUMN ds_atributo_defesa VARCHAR(50) DEFAULT 'AGI'");
        }
        ...
        // Reclassificar Poderes e Rituais de Ordem Paranormal (id_sistema = 1)
        $pdo->exec("UPDATE tb_habilidade SET tp_habilidade = 'poder' WHERE id_sistema = 1 AND (ds_habilidade LIKE 'Ritual%' OR nm_habilidade IN ('Armamento Paranormal', 'Escudo Paranormal', 'Impacto Paranormal', 'Destruir Paranormal', 'Luz Sagrada', 'Ancoragem', 'Canalizar Elemento'))");
...
<head>
    ...
    <style>
        ...
        body.tema-ordem-paranormal .premium-header,
        body.tema-ordem-paranormal .nivel-deslocamento-row {
            background: rgba(20, 8, 8, 0.78) !important;
            border: 1.5px solid rgba(255, 50, 50, 0.28) !important;
            box-shadow: 0 10px 40px rgba(255, 50, 50, 0.22) !important;
        ...
        .p-name { cursor: pointer; transition: all 0.2s; }
        .p-name:hover { text-shadow: 0 0 8px var(--premium-accent, #9d7aff); color: #fff !important; }
        body.tema-ordem-paranormal .p-name:hover { text-shadow: 0 0 8px #ff3232 !important; }
        ...
        #result-card {
            ...
            border: 2px solid var(--premium-accent, #9d7aff);
            border-radius: 24px;
            padding: 40px 50px;
            text-align: center;
            box-shadow:
        ...
        body.tema-ordem-paranormal #result-card {
            border-color: #ff3232 !important;
            box-shadow: 0 0 25px rgba(255, 50, 50, 0.4) !important;
        }
        ...
        #result-label {
            ...
            font-size: 0.8rem;
            letter-spacing: 4px;
            color: var(--premium-accent, #9d7aff);
            text-transform: uppercase;
            margin-bottom: 12px;
            opacity: 0.85;
        }
        body.tema-ordem-paranormal #result-label {
            color: #ff3232 !important;
        }
        ...
        body.tema-ordem-paranormal #result-total {
            text-shadow: 0 0 30px rgba(255, 50, 50, 0.6), 0 0 60px rgba(255, 50, 50, 0.3) !important;
        }
        ...
        body.tema-ordem-paranormal #result-breakdown .val { color: #ff6666; }
        #result-breakdown .op  { color: #666; margin: 0 5px; }
    ...
    <script>
        const DDDICE_API_KEY = "loKtWZoIgQgepNUC44LpaeYJZdAU7mqs1S9FxHTabc00dd7b";
        const DDDICE_ROOM_SLUG = "mF9ol6O";
    </script>
...
<body class="ficha-body <?= $classeBackground ?>">
    ...
    <div class="ficha-container-master">
        ...
        <div class="ficha-layout-premium">
            ...
            <div class="nivel-deslocamento-row">
                ...
                <div class="extra-stat-box">DESLOCAMENTO <span class="line-text" contenteditable="true" data-campo="ds_deslocamento" style="border-bottom:1px dashed rgba(255,255,255,0.3); outline:none; cursor:text; min-width:20px; display:inline-block; text-align:center; font-weight:700;"><?= htmlspecialchars($personagem['ds_deslocamento'] ?? '9 m / 6 q') ?></span></div>
            </div>
            ...
            <section class="premium-main">
                ...
                <div class="premium-col-left">
                    ...
                    <!-- COMBATE (DEFESA ETC) -->
                    <?php
                    $soma_escudos_adicionais = 0;
                    $tem_escudos_adicionais = false;
                    foreach ($status_defesas as $d) {
                        if ($d['id_status_sistema'] !== 'defesa') {
                            $soma_escudos_adicionais += (int)$d['qt_atual'];
                            $tem_escudos_adicionais = true;
                        }
                    }
                    if (!$tem_escudos_adicionais) {
                        $soma_escudos_adicionais = 10;
                    }
                    ?>
                    <div class="premium-combat-section <?= (count($status_defesas) >= 2) ? 'defesa-multi-shields' : '' ?>">
                        <?php if (count($status_defesas) >= 2): ?>
                            <div class="escudos-row" style="display: flex; gap: 15px; align-items: center; width: 100%; justify-content: center; margin-bottom: 15px; flex-wrap: wrap;">
                                <?php foreach ($status_defesas as $d): ?>
                                    <div class="shield-wrapper" style="display: flex; flex-direction: column; align-items: center; gap: 8px;">
                                        ...
                                        <span class="shield-label" style="font-size: 0.75rem; font-weight: 800; color: rgba(255,255,255,0.75); text-transform: uppercase; letter-spacing: 0.5px; text-align: center; text-shadow: 0 1px 3px rgba(0,0,0,0.5);"><?= htmlspecialchars($d['nm_status']) ?></span>
                                    </div>
                                <?php endforeach; ?>
                            </div>
                        <?php else: ?>
                            <?php if (empty($status_defesas)): ?>
                                <div class="shield-wrapper" style="display: flex; flex-direction: column; align-items: center; gap: 8px;">
                                    ...
                                    <span class="shield-label" style="font-size: 0.75rem; font-weight: 800; color: rgba(255,255,255,0.75); text-transform: uppercase; letter-spacing: 0.5px; text-align: center; text-shadow: 0 1px 3px rgba(0,0,0,0.5);">DEFESA</span>
                                </div>
                            <?php else: ?>
                                <?php foreach ($status_defesas as $d): ?>
                                    <div class="shield-wrapper" style="display: flex; flex-direction: column; align-items: center; gap: 8px;">
                        ...
                        <div class="defesa-details" style="width: 100%;">
                            <div class="defesa-formula">
                                <span class="formula-label">DEFESA</span>
                                <span class="formula-text">= <span id="formula-soma-escudos"><?= $soma_escudos_adicionais ?></span> + 
                                    <select id="seletor-atributo-defesa" style="background: none; border: none; color: #fff; font-weight: 800; font-size: 1.1rem; cursor: pointer; outline: none; border-bottom: 1px dashed rgba(255,255,255,0.4); padding: 0 5px;">
                                        <?php foreach ($atributos as $at): ?>
                                            <option value="<?= htmlspecialchars($at['ds_abreviacao']) ?>" <?= (($personagem['ds_atributo_defesa'] ?? 'AGI') === $at['ds_abreviacao']) ? 'selected' : '' ?> style="background: #110e1a; color: #fff;">
                                                <?= htmlspecialchars($at['ds_abreviacao']) ?>
                                            </option>
                                        <?php endforeach; ?>
                                    </select> + 
    ...
    <!-- Pop-up de resultado de rolagens (sem histórico) -->
    <div id="result-popup">
        <div id="result-backdrop" onclick="fecharResultado()"></div>
        <div id="result-card" onclick="fecharResultado()">
            <div id="result-label">ROLAGEM</div>
            <div id="result-total">--</div>
            <div id="result-breakdown">--</div>
            <div id="result-dismiss" style="color: #666; font-size: 0.75rem; margin-top: 15px; letter-spacing: 1px; text-transform: uppercase;">CLIQUE EM QUALQUER LUGAR PARA FECHAR</div>
        </div>
    </div>
