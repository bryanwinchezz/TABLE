...
if (isset($_SESSION['usuario'])) {
    try {
        ...
        $stmtUsr = $pdo->prepare("SELECT fl_plano_mapas, fl_plano_sistemas, fl_plano_completo, tp_cargo, dt_desistencia_mestre FROM tb_usuario WHERE id_usuario = ? LIMIT 1");
        $stmtUsr->execute([$_SESSION['usuario']['id']]);
        $dadosUsr = $stmtUsr->fetch();
        if ($dadosUsr) {
            $possuiMapas = ((int)$dadosUsr['fl_plano_mapas'] === 1);
            $possuiSistemas = ((int)$dadosUsr['fl_plano_sistemas'] === 1);
            $possuiCompleto = ((int)$dadosUsr['fl_plano_completo'] === 1);
            $cargoLower = strtolower($dadosUsr['tp_cargo'] ?? 'jogador');
            $desistencia = $dadosUsr['dt_desistencia_mestre'];
            $jaMestre = ($cargoLower === 'mestre' || ($cargoLower === 'admin' && $desistencia === null));
        }
    } catch (Exception $e) {}
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['resgatar_chave_ajax'])) {
    ...
    // Tratamento especial do cupom promocional compartilhado
    if (strtoupper($chave) === 'TBL-ETEC-2026-TCC') {
        $codigoComposto = 'TBL-ETEC-2026-TCC-' . $_SESSION['usuario']['id'];
        try {
            $pdo = Database::getConexao();
            // Verificar se o usuário já resgatou este cupom promocional
            $stmtProm = $pdo->prepare("SELECT id_chave FROM tb_chave_ativacao WHERE ds_codigo = ? LIMIT 1");
            $stmtProm->execute([$codigoComposto]);
            $cupomResgatado = $stmtProm->fetch();

            if ($cupomResgatado) {
                echo json_encode(['sucesso' => false, 'mensagem' => 'Você já resgatou este cupom promocional em sua conta!']);
                exit;
            }

            // Registrar o uso do cupom promocional no banco de dados
            $stmtIns = $pdo->prepare("
                INSERT INTO tb_chave_ativacao (ds_codigo, tp_plano, fl_usada, id_usuario_comprador, id_usuario_ativador, dt_ativacao)
                VALUES (?, 'completo', 1, ?, ?, NOW())
            ");
            $stmtIns->execute([$codigoComposto, $_SESSION['usuario']['id'], $_SESSION['usuario']['id']]);

            // Habilitar plano completo para o usuário (Qualidade Absoluta - preservando admin se aplicável)
            $stmtCargo = $pdo->prepare("SELECT tp_cargo FROM tb_usuario WHERE id_usuario = ? LIMIT 1");
            $stmtCargo->execute([$_SESSION['usuario']['id']]);
            $cargoAtual = $stmtCargo->fetchColumn();
            
            $novoCargo = ($cargoAtual === 'admin') ? 'admin' : 'mestre';

            $stmtAct = $pdo->prepare("UPDATE tb_usuario SET fl_plano_completo = 1, fl_plano_mapas = 1, fl_plano_sistemas = 1, tp_cargo = ?, dt_desistencia_mestre = NULL WHERE id_usuario = ?");
            $stmtAct->execute([$novoCargo, $_SESSION['usuario']['id']]);
            ...
            echo json_encode([
                'sucesso' => true,
                'mensagem' => 'Sua chave de ativação foi resgatada com sucesso! O Plano Completo foi desbloqueado.',
                'plano' => 'Plano Completo'
            ]);
            exit;
        } catch (Exception $e) {
            echo json_encode(['sucesso' => false, 'mensagem' => 'Erro ao processar cupom promocional: ' . $e->getMessage()]);
            exit;
        }
    }
    ...
    try {
        ...
        // Ativar as flags correspondentes na conta do usuário (Qualidade Absoluta - preservando admin se aplicável)
        $stmtCargo = $pdo->prepare("SELECT tp_cargo FROM tb_usuario WHERE id_usuario = ? LIMIT 1");
        $stmtCargo->execute([$_SESSION['usuario']['id']]);
        $cargoAtual = $stmtCargo->fetchColumn();
        ...
        if ($tp_plano === 'mapas') {
            $stmtAct = $pdo->prepare("UPDATE tb_usuario SET fl_plano_mapas = 1, tp_cargo = ?, dt_desistencia_mestre = NULL WHERE id_usuario = ?");
        } elseif ($tp_plano === 'sistemas') {
            $stmtAct = $pdo->prepare("UPDATE tb_usuario SET fl_plano_sistemas = 1, tp_cargo = ?, dt_desistencia_mestre = NULL WHERE id_usuario = ?");
        } else {
            $stmtAct = $pdo->prepare("UPDATE tb_usuario SET fl_plano_completo = 1, fl_plano_mapas = 1, fl_plano_sistemas = 1, tp_cargo = ?, dt_desistencia_mestre = NULL WHERE id_usuario = ?");
        }
        $stmtAct->execute([$novoCargo, $_SESSION['usuario']['id']]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['virar_mestre_plano'])) {
    ...
    try {
        ...
        $stmt = $pdo->prepare("UPDATE tb_usuario SET tp_cargo = ?, dt_desistencia_mestre = NULL WHERE id_usuario = ?");
        $stmt->execute([$novoCargo, $_SESSION['usuario']['id']]);
        $_SESSION['usuario']['cargo'] = $novoCargo;
...
<body class="pagina-inicial">
                        ...
                        ?>
                            <div class="chave-usuario-card" style="background: rgba(19, 16, 34, 0.8); border: 1px solid <?= $usada ? 'rgba(255,255,255,0.05)' : 'var(--cor-borda-card)' ?>; border-radius: 16px; padding: 20px; display: flex; flex-direction: column; gap: 15px; position: relative; transition: all 0.3s; opacity: <?= $usada ? 0.75 : 1 ?>;">
                                ...
                                <div class="chave-codigo-display" style="background: rgba(0,0,0,0.3); padding: 12px 15px; border-radius: 10px; display: flex; justify-content: space-between; align-items: center; border: 1px solid rgba(255,255,255,0.05);">
                                    <?php 
                                        $codigoExibido = preg_replace('/-ETEC-2026-TCC-\d+$/i', '-ETEC-2026-TCC', $ch['ds_codigo']);
                                    ?>
                                    <span class="codigo-texto" data-codigo="<?= htmlspecialchars($codigoExibido) ?>" style="font-family: monospace; font-size: 1.05rem; letter-spacing: 1.5px; color: var(--cor-branco);">••••-••••-••••-••••</span>
                                    <div style="display: flex; gap: 10px;">
                                        ...
                                        <button type="button" onclick="copiarChaveCodigo('<?= htmlspecialchars($codigoExibido) ?>', this)" class="btn-copiar-ch" style="background: transparent; border: none; color: var(--cor-texto-suave); cursor: pointer; font-size: 1rem; transition: color 0.2s;" title="Copiar Chave">
                                ...
                                <div style="display: flex; justify-content: space-between; align-items: center; margin-top: 5px;">
                                    ...
                                    <?php else: ?>
                                        <div style="display: flex; flex-direction: column; align-items: flex-end; gap: 2px;">
                                            ...
                                            <?php if (strpos(strtoupper($ch['ds_codigo']), 'TBL-ETEC-2026-TCC') !== false): ?>
                                                <span style="font-size: 0.72rem; color: #2ecc71; font-weight: 700; display: flex; align-items: center; gap: 4px;">
                                                    <i class="fas fa-infinity"></i> Vitalício
                                                </span>
                                            <?php else: 
                                                $dt_expiracao = date('d/m/Y', strtotime($ch['dt_ativacao'] . ' + 30 days'));
                                                $timestamp_exp = strtotime($ch['dt_ativacao'] . ' + 30 days');
                                                $dias_restantes = ceil(($timestamp_exp - time()) / 86400);
                                                if ($dias_restantes < 0) $dias_restantes = 0;
                                            ?>
                                                <span style="font-size: 0.72rem; color: <?= $dias_restantes <= 5 ? '#e74c3c' : 'var(--cor-texto-suave)' ?>; font-weight: 600;">
                                                    Expira em: <?= $dt_expiracao ?> (<?= $dias_restantes ?>d)
                                                </span>
                                            <?php endif; ?>
                                        </div>
                                    <?php endif; ?>
            ...
            window.iniciarCheckout = function(planoId) {
                ...
                .then(data => {
                    if (data.sucesso) {
                        ...
                        TableModal.alert(data.mensagem || 'Ocorreu um erro ao iniciar o checkout.', 'Erro de Checkout', 'error');
                    }
                })
                .catch(error => {
                    btn.disabled = false;
                    ...
                    TableModal.alert('Erro de conexão ao processar checkout.', 'Erro de Conexão', 'error');
                });
            };
            ...
            window.abrirModalSerMestre = function() {
                ...
                .then(data => {
                    ...
                    if (data.sucesso) {
                        ...
                        TableModal.alert(data.mensagem || 'Ocorreu um erro ao promover para Mestre.', 'Erro de Promoção', 'error');
                    }
                })
                .catch(error => {
                    btnGratis.disabled = false;
                    btnGratis.innerHTML = originalText;
                    console.error('Error:', error);
                    TableModal.alert('Erro de conexão ao promover usuário.', 'Erro de Conexão', 'error');
                });
            };
    ...
    <script>
        ...
        function resgatarChaveRapido(codigo, btn) {
            ...
            .then(data => {
                if (data.sucesso) {
                    ...
                    TableModal.alert(data.mensagem, 'Sucesso', 'success');
                    setTimeout(() => { window.location.reload(); }, 2000);
                } else {
                    btn.disabled = false;
                    btn.innerHTML = origText;
                    TableModal.alert(data.mensagem, 'Erro', 'error');
                }
            })
            .catch(err => {
                btn.disabled = false;
                btn.innerHTML = origText;
                TableModal.alert('Erro de conexão ao ativar.', 'Erro de Conexão', 'error');
            });
        }
        ...
        function fecharModalChaveComprada(codigo) {
            ...
            .then(data => {
                if (data.sucesso) {
                    TableModal.alert('Chave ativada com sucesso na sua conta!', 'Sucesso', 'success');
                }
                setTimeout(() => { window.location.href = 'planos.php'; }, 2000);
            })
            .catch(() => {
