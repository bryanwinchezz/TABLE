...
function inicializarAvatar() {
    ...
    input.onchange = async (e) => {
        ...
        if (typeof abrirCropperModal === 'function') {
            abrirCropperModal(file, 1, async (croppedBlob, croppedBase64) => {
...
function filtrarListaModal(termo, tipo, isCatalogo = false) {
    ...
    const filtrada = base.filter(i => {
        ...
        // Se for o catálogo de inventário, aplica filtro de categoria
        if (isCatalogo && tipo === 'inventario') {
            if (filtroInventarioAtiva !== 'Todos') {
                const catItem = (i.tp_item || '').trim().toLowerCase();
                if (catItem !== filtroInventarioAtiva.toLowerCase()) return false;
            }
        }
...
function renderizarListaComponentes(lista, tipo, isCatalogo = false) {
    ...
    return lista.map((item, idx) => {
        ...
        return `
            <div class="modal-list-item" style="background:rgba(255,255,255,0.03); border:1px solid rgba(255,255,255,0.05); border-radius:12px; margin-bottom:10px; overflow:hidden; display:flex; flex-direction:column; align-items:stretch;">
...
function abrirCatalogo(tipo) {
    ...
    if (tipo === 'habilidades') {
        ...
        if (typeof SISTEMA_ITENS !== 'undefined' && Array.isArray(SISTEMA_ITENS)) {
            SISTEMA_ITENS.forEach(item => {
                let cat = item.tp_item ? item.tp_item.trim() : '';
                if (cat) {
                    const catNormalizada = cat.charAt(0).toUpperCase() + cat.slice(1).toLowerCase();
                    if (!categoriasUnicas.includes(catNormalizada)) {
                        categoriasUnicas.push(catNormalizada);
                    }
                }
            });
        }
        ...
        if (categoriasUnicas.length > 0) {
            const categorias = ['Todos', ...categoriasUnicas.sort()];
            filtersHtml = `
                <div class="pills-container">
                    ${categorias.map(c => `
                        <button class="pill-btn ${c === 'Todos' ? 'ativo' : ''}" onclick="toggleFiltroInventario('${c}', this)">${c}</button>
                    `).join('')}
                </div>
            `;
        }
...
function toggleFiltroInventario(categoria, btn) {
    filtroInventarioAtiva = categoria;
    document.querySelectorAll('.pills-container .pill-btn').forEach(b => b.classList.remove('ativo'));
    btn.classList.add('ativo');
    triggerFiltroCatalogo('inventario');
}
...
function inicializarFichaEditavel() {
    ...
    if (nivelEl) {
        let nivelAnterior = 1;
        nivelEl.addEventListener('focus', () => {
            nivelAnterior = parseInt(nivelEl.textContent.replace(/\D/g, '')) || 1;
        });
        nivelEl.addEventListener('blur', () => {
            const val = parseInt(nivelEl.textContent.replace(/\D/g, '')) || 1;
            nivelEl.textContent = val;
            const difNivel = val - nivelAnterior;
            salvarDados('personagem_campo', 'qt_nivel', val);
            recalcularValoresFicha(difNivel);
        });
    ...
    // 6. Ouvir mudança de atributo para Defesa
    const seletorAttr = document.getElementById('seletor-atributo-defesa');
    if (seletorAttr) {
        seletorAttr.addEventListener('change', () => {
            const val = seletorAttr.value;
            salvarDados('personagem_campo', 'ds_atributo_defesa', val);
            recalcularValoresFicha();
        });
    }
}

function recalcularValoresFicha(difNivel = 0) {
    ...
    // Obter o atributo de defesa selecionado no seletor
    const seletorAttr = document.getElementById('seletor-atributo-defesa');
    const attrSigla = seletorAttr ? seletorAttr.value.trim().toUpperCase() : 'AGI';
    const valorAttr = getAttr([attrSigla]);

    // 4. Atualizar as barras no front-end e salvar no banco via AJAX
    const atualizarBarraMax = (label, novoMax) => {
        ...
        if (unit) {
            ...
            if (spanMax) {
                const maxAnterior = parseInt(spanMax.textContent) || 0;
                spanMax.textContent = novoMax;
                let atual = parseInt(spanAtual.textContent) || 0;
                
                // Se o nível aumentou, adicionamos a diferença do valor máximo ao atual também
                if (difNivel > 0) {
                    const incremento = novoMax - maxAnterior;
                    if (incremento > 0) {
                        atual += incremento;
                    }
                }
    ...
    if (isOrdem) {
        // Calcular PV, Sanidade e PE máximos baseados das classes oficiais de Ordem Paranormal
        let maxPV = 8 + vig;
        let maxSAN = 8;
        let maxPE = 1;
        ...
        // Regra Geral: Começa com 10/10, ao subir nível eleva +10 (ou seja, Nivel * 10)
        const maxCalculado = nivel * 10;
        document.querySelectorAll('.bar-unit').forEach(unit => {
            const label = unit.dataset.campo.toLowerCase();
            let bonusAtributo = 0;
            
            // escolha do atributo de defesa no select, podendo aumentar alguma barra de saude de acordo com aquele atributo
            if (attrSigla.startsWith('VIG') || attrSigla === 'VIGOR') {
                if (label.includes('vida') || label.includes('total') || label.includes('vd') || label.includes('vt')) {
                    bonusAtributo = valorAttr;
                }
            } else if (attrSigla.startsWith('PRE') || attrSigla === 'PRESENÇA' || attrSigla === 'PRESENCA') {
                if (label.includes('esforço') || label.includes('esforco') || label.includes('pe') || label.includes('energia')) {
                    bonusAtributo = valorAttr;
                }
            } else if (attrSigla.startsWith('INT') || attrSigla === 'INTELECTO' || attrSigla === 'INTELIGÊNCIA' || attrSigla === 'INTELIGENCIA') {
                if (label.includes('sanidade') || label.includes('mental')) {
                    bonusAtributo = valorAttr;
                }
            }
            
            atualizarBarraMax(unit.dataset.campo, maxCalculado + bonusAtributo);
        });
    }

    // 5. Calcular Defesa Dinâmica: Soma dos Escudos + Atributo Escolhido + Equipamento + Outros
    ...
    // Calcular soma dos escudos adicionais (aqueles que não são a defesa principal)
    let somaEscudos = 0;
    const escudosAdicionais = Array.from(document.querySelectorAll('.defesa-shield-box')).filter(box => {
        const idStatus = box.dataset.idStatus;
        const title = (box.title || '').toLowerCase();
        return idStatus !== 'defesa' && !title.includes('defesa') && !title.includes('def ');
    });
    
    if (escudosAdicionais.length === 0) {
        somaEscudos = 10; // 10 padrão
    } else {
        escudosAdicionais.forEach(box => {
            const numEl = box.querySelector('.shield-number');
            somaEscudos += numEl ? (parseInt(numEl.textContent) || 0) : 0;
        });
    }

    const totalDefesa = somaEscudos + valorAttr + equip + outros;

    // Atualizar a fórmula visual na UI (o número da frente)
    const spanSomaEscudos = document.getElementById('formula-soma-escudos');
    if (spanSomaEscudos) {
        spanSomaEscudos.textContent = somaEscudos;
    }

    // Atualizar escudo de defesa na UI e no Banco de Dados
    const shieldDefesaBox = document.querySelector('.defesa-shield-box[data-id-status="defesa"]') ||
                            Array.from(document.querySelectorAll('.defesa-shield-box')).find(
                                box => (box.title || '').toLowerCase().includes('defesa') || 
                                       box.dataset.idStatus === 'defesa'
                            );
    if (shieldDefesaBox) {
        const shieldDefesa = shieldDefesaBox.querySelector('.shield-number');
        if (shieldDefesa) {
            shieldDefesa.textContent = totalDefesa;
        }
        const idStatus = shieldDefesaBox.dataset.idStatus;
        if (idStatus === 'defesa') {
    ...
    // 6. Calcular Esquiva Dinâmica: Defesa + Atributo de Defesa
    const esquivaValEl = document.querySelector('.defesa-stats-row .val[data-campo="qt_esquiva"]');
    if (esquivaValEl) {
        const totalEsquiva = totalDefesa + valorAttr;
...
async function initDddice() {
    ...
    if (!canvas || !window.DDDICE_API_KEY || !window.DDDICE_ROOM_SLUG) {
        return;
    }
    ...
    try {
        ...
        // Carregar primeiro tema ativo da API do dddice
        try {
            const resp = await fetch('https://dddice.com/api/1.0/dice-box', {
                headers: { 'Authorization': `Bearer ${window.DDDICE_API_KEY}` }
            });
            const data = await resp.json();
            if (data && data.data && data.data.length > 0) {
                dddiceTheme = data.data[0].id;
            }
        } catch (e) {
            console.warn("Erro ao buscar tema dddice, usando padrão:", e);
        }
    } catch (err) {
        console.error("Falha ao conectar dddice:", err);
    }
}

function inicializarCliquesPericias() {
    document.querySelectorAll('.p-row').forEach(row => {
        const nameEl = row.querySelector('.p-name');
        if (!nameEl) return;
        
        nameEl.addEventListener('click', () => {
            const nomePericia = nameEl.textContent.trim();
            const bonusEl = row.querySelector('.p-bonus');
            const bonusText = bonusEl ? bonusEl.textContent.replace(/[^\d-]/g, '') : '0';
            const bonusVal = parseInt(bonusText) || 0;
            
            rolarPericia(nomePericia, bonusVal);
        });
    });
}

let rolandoDado = false;
async function rolarPericia(nomePericia, bonus) {
    ...
    labelEl.textContent = `Rolagem de ${nomePericia}`;
    totalEl.textContent = "--";
    breakdownEl.textContent = "Rolando...";
    popup.classList.add('show');
    
    let d20Resultado = 0;
    let viaDddice = false;
    
    if (dddiceConectado && dddiceSDK) {
        try {
            const rollPromise = dddiceSDK.roll([{ type: 'd20', theme: dddiceTheme }]);
            const res = await rollPromise;
            if (res && res.values && res.values.length > 0) {
                d20Resultado = parseInt(res.values[0].value) || 1;
                viaDddice = true;
            } else {
                d20Resultado = Math.floor(Math.random() * 20) + 1;
            }
        } catch (err) {
            console.warn("Erro na rolagem 3D, usando rolagens JS:", err);
            d20Resultado = Math.floor(Math.random() * 20) + 1;
        }
    } else {
        d20Resultado = Math.floor(Math.random() * 20) + 1;
    ...
    const total = d20Resultado + bonus;
    ...
    setTimeout(() => {
        ...
        const opSinal = bonus >= 0 ? '+' : '-';
        const absBonus = Math.abs(bonus);
        breakdownEl.innerHTML = `<span class="val">d20 (${d20Resultado})</span><span class="op">${opSinal}</span><span class="val">${absBonus}</span><span class="op">=</span><span class="val">${total}</span>`;
